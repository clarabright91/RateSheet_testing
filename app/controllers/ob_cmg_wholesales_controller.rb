        ...
        (10..87).each do |r|
          ...
          if ((row.compact.count > 1) && (row.compact.count <= 4))
            ...
            (0..max_column_section).each do |max_column|
              ...
              @program.save
                @program.update(term: @term,loan_type: loan_type,loan_purpose: "Purchase",streamline: @streamline,fha: @fha, va: @va, usda: @usda, full_doc: @full_doc, jumbo_high_balance: @jumbo_high_balance, arm_basic: @arm_basic)
                # @program.adjustments.destroy_all
                @block_hash = {}
                key = ''
                if @program.term.present?
